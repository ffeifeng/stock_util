...
用法：
  python 回测-站回MA30_基础版.py                     # 默认回测2026年1月
  python 回测-站回MA30_基础版.py 20260101 20260131   # 指定起止日期
"""
...
OUT_PATH = os.path.join(OUT_DIR, f'回测-站回MA30_基础版_{START_DATE}_{END_DATE}.txt')
...
def run():
    ...
    out('=' * 80)
    out(f'  回测策略：站回MA30 基础版（离MA30<=>{ABOVE_MAX}%，无额外过滤）')
    out(f'  回测区间：{START_DATE} ~ {END_DATE}')
    out(f'  信号逻辑：前天上方->昨天跌破->今天站回(<=>{ABOVE_MAX}%) -> 当天收盘买入')
    out(f'  扫描标的：{total_stocks} 只股票    触发信号：{len(all_signals)} 次')
    out('=' * 80)
    out()
    out('  【持有收益汇总】')
    ...
    if cur_date: flush_date(cur_date)
    out()
